...
final class Chat: Identifiable {
    ...
    init(id: UUID = UUID(), title: String, createdAt: Date = Date(), isPinned: Bool = false, messages: [Message] = []) {
        self.id = id
        self.title = title
...
final class Message: Identifiable {
    ...
    var createdAt: Date
    var chat: Chat?
    // Optional reaction: "up" | "down"
    var reaction: String?

    init(id: UUID = UUID(), role: String, content: String, createdAt: Date = Date(), chat: Chat? = nil, reaction: String? = nil) {
        self.id = id
        self.role = role
...
final class AppSettings: Identifiable {
    ...
    // Chat input: press Enter to send (otherwise, Return inserts newline)
    var enterToSend: Bool
    // Preserve drafts per chat when switching threads
